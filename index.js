...
function sortCarInventory(inventory) {
  ...
  for (let i = 0; i < carModels.length; i++)
    for (let v  = 0; v < inventory.length; v++)
      if (inventory[v].car_model == carModels[i])
        sortedInv.push(inventory[v]);
...
function getOlderCars(inventory, max_year) {
  ...
  if( max_year >= inventory[i].car_year )
  
    oldYears.push(inventory[i]);
...
function carMaker(odoNum) {
  return{
    ...
    drive: function(distNum){
      let newOdo = this.odometer + distNum;
